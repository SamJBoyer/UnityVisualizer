...
public class CSNode
{
    ...
    public CSNode(string overrideString = null)
    {
        ...
        else //no override input, and this node is being run from BRAND
        {
            ...
            _serverIP = args[4];
            _serverPort = args[6];
            ConnectFromBRAND(_serverIP, _serverPort).Wait(); //connect async. currently doesn't allow you to connect via socket
    ...
    {
        _connectionString = serverSocket != null ? serverSocket : $"{serverIP}:{serverPort}";
